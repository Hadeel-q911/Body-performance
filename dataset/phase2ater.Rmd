...
+----------+-------------------------------+----------+----------+
| name     | description                   | data     | possible |
|          |                               | type     | value    |
|          |                               |          |          |
+==========+===============================+==========+==========+
...
|          |                               |          |          |
+----------+-------------------------------+----------+----------+
| weight_kg| The person's weight in Kg     | Numeric  | 2        |
|          |                               |          | 6.3-138  |
+----------+-------------------------------+----------+----------+
| body     | the amount of essential fat . | Numeric  | 3        |
| fat\_%   |                               |          | %-78.4%  |
+----------+-------------------------------+----------+----------+
| diastolic| measures pressure the blood   | Numeric  | 0-156    |
|          | vessels when the heart is at  |          |          |
|          | rest                          |          |          |
+----------+-------------------------------+----------+----------+
|          | measures pressure in the      | Numeric  | 0-201    |
| Systolic | arteries when the heart beats |          |          |
|          | in minutes                    |          |          |
+----------+-------------------------------+----------+----------+
| gripForce| fingers flexibility tests     | Numeric  | 0-70.5   |
|          |                               |          |          |
+----------+-------------------------------+----------+----------+
| sit and  | measures flexibility in       | Numeric  | -25-213  |
| bend for | sitting and bending forward   |          |          |
| ward_cm  | in centimeters.               |          |          |
+----------+-------------------------------+----------+----------+
| sit-ups  | measures the strength and     | Numeric  | 0-80     |
| counts_cm| endurance of the abdominals   |          |          |
|          | and hip-flexor muscles in c   |          |          |
|          | entimeter.                    |          |          |
+----------+-------------------------------+----------+----------+
| broad    | It is a method of measuring   | Numeric  | 0-303    |
| jump_cm  | how far a person can jump     |          |          |
...
The chosen partitioning method for the dataset involves employing the train:test split technique. This approach entails dividing the dataset into three subsets: 70% for training and 30% for testing, 75% for training and 25% for testing, and finally, 80% for training and 20% for testing. The utilization of the train:test split method is a widely adopted practice in machine learning. This strategic partitioning aids in optimizing the effective use of available data for both training and testing purposes. By implementing this method, the risk of overfitting is mitigated, as it ensures a dedicated dataset for the evaluation of model performance.
...
Root Node - sit and bend forward_cm: This divides the tree into two main branches or subtrees:
Right Subtree: if the sit and bend forward_cm < 0.73 .
Left Subtree: if the sit and bend forward_ cm >=0.73

Within the right subtree:
If the ‘sit and bend forward_cm’ >=1.3 , the tree splits based on the ‘body_fat’ attribute. It checks whether the ‘body_fat’ < 0.31 

For the left subtree : 
If the ‘sit and bend forward_cm’ >=0.085 the tree splits based on ‘sit up counts’ and if ‘sit and bend forward_cm’ <0.085 the tree splits based on ‘body_fat’
...
: Table 2: Statistics for partition 70/30 (Information Gain)
...
For the left subtree : 
If the ‘sit and bend forward_cm’ >=0.012 the tree splits based on ‘sit up counts’ and if ‘sit and bend forward_cm’ <0.012 the tree splits based on ‘body_fat’
...
: Table 3: Statistics for partition 70/30 (Gain ratio)
...
For the left subtree : 
If the ‘sit and bend forward_cm’ >=0.65 the tree splits based on ‘sit up counts’ and if ‘sit and bend forward_cm’ <0.65 the tree splits based on ‘body_fat’
...
: Table 4: Statistics for partition 70/30 (Gini index)
...
For the left subtree : 
If the ‘sit and bend forward_cm’ >=0.048 the tree splits based on ‘sit up counts’ and if ‘sit and bend forward_cm’ <0.048 the tree splits based on ‘body_fat’
...
: Table 6: Statistics for partition 75/25 (information gain)
...
For the left subtree : 
If the ‘sit and bend forward_cm’ >=0.036 the tree splits based on ‘sit up counts’ and if ‘sit and bend forward_cm’ <0.036 the tree splits based on ‘body_fat’
...
: Table 7: Statistics for partition 75/25 (Gain ratio)
...
: Table 8: Statistics for partition 75/25 (Gini index)
...
# Print the decision tree
rpart.plot(rpart_model, box.palette = "auto", nn = TRUE, main = "Decision Tree for ID3 80/20")
```
...
: Table 9: Statistics for partition 80/20 (Information Gain)
...
: Table 10: Statistics for partition 80/20 (Gain ratio)
...
: Table 11: Statistics for partition 80/20 (Gini index)
...
: Table 12: 80/20 Comparison Classification algorithm
...
: Table 13: 75/25 Comparison Classification algorithm
...
: Table 14: 70/30 Comparison Classification algorithm
