...
### ^[[1]](https://www.kaggle.com/datasets/kukuroo3/body-performance-data)^3.1: Data Source :
...
| fat\_%  |                                      | merical | %-78.4% |
+---------+--------------------------------------+---------+---------+
| di      | measures pressure the blood vessels  | nu      | 0-156   |
| astolic | when the heart is at rest            | merical |         |
+---------+--------------------------------------+---------+---------+
| s       | measures pressure in the arteries    | nu      | 0-201   |
| ystolic | when the heart beats in minutes      | merical |         |
+---------+--------------------------------------+---------+---------+
| gr      | fingers f lexibility tests           | nu      | 0-70.5  |
| ipForce |                                      | merical |         |
+---------+--------------------------------------+---------+---------+
| sit and | measures f lexibility in sitting and | nu      | -25-213 |
| bend    | bending forward in ce ntimeters.     | merical |         |
| for     |                                      |         |         |
| ward_cm |                                      |         |         |
+---------+--------------------------------------+---------+---------+
| sit-ups | measures the strength and endurance  | nu      | 0-80    |
| co      | of the abdominals and hip-flexor     | merical |         |
| unts_cm | muscles in c entimeter.              |         |         |
+---------+--------------------------------------+---------+---------+
| broad   | It is a method of measuring how far  | nu      | 0-303   |
| jump_cm | a person can jump from a standing    | merical |         |
|         | position to a landing position.      |         |         |
+---------+--------------------------------------+---------+---------+
...
### ^[[2]](https://www.kaggle.com/code/codeguyas101/body-performance-data-eda),[[2]](https://www.kaggle.com/code/shivanirana63/guide-to-complete-statistical-analysis)^4.2 Understanding the dataset using graph representations:
...
### ^[[3]](https://www.rdocumentation.org/packages/arules/versions/1.6-4/topics/discretize)^ 4.7 Discretization :
...
### ^[[4]](https://rpubs.com/camguild/803096)^5.1 Classification:
...
### ^[[5]](https://rpubs.com/odenipinedo/cluster-analysis-in-R)^ 5.2 Clustering:
...
| Clusters      |               |               |               |
+:=============:+:=============:+:=============:+:=============:+
| Average       | cluter        | clute r1=0.58 | clute r1=0.55 |
| Silhouette    | 1=0.56,       | , clute       | , clute       |
| width for     |               | r2=0.60 ,     | r2=0.30 ,     |
| each cluter   | clute r2=0.67 | clute r3=0.41 | clute r3=0.37 |
|               |               |               | ,clute        |
|               |               |               | r4=0.46       |
+---------------+---------------+---------------+---------------+
...
| width         |               |               |               |
+---------------+---------------+---------------+---------------+
| Total within- | 5 91064.6     | 3 10351.6     | 2 30049.1     |
| cluster sum   |               |               |               |
| of square     |               |               |               |
+---------------+---------------+---------------+---------------+
...
## ^[[6]](https://www.kaggle.com/code/shivanirana63/guide-to-complete-statistical-analysis)^7. Finding:

Considering the project's goal and the problem at hand, the
classification task emerged as the more advantageous approach for
analysis. By assigning class labels to the data, the classification task
enabled the identification of influential factors significantly
impacting body scores. The decision tree algorithm provided valuable
insights into these factors, shedding light on their importance and
relationships.

In contrast, the clustering task encountered challenges primarily due to
the presence of overlapping clusters. While the clustering task
facilitated the understanding of similarities among body performance
data points, it did not directly contribute to problem-solving or goal
attainment.

In conclusion, the classification task proved to be a more helpful and
accurate approach for addressing the problem and achieving the project's
goal. It allowed for the identification of factors influencing body
scores, providing valuable insights for optimizing training programs.
Conversely, the clustering task, while offering some insights into data
patterns, did not directly contribute to problem-solving or goal
achievement.
...
[2].[Body Performance Data
EDA](https://www.kaggle.com/code/codeguyas101/body-performance-data-eda)
, [guide to complete statistical
analysis](https://www.kaggle.com/code/shivanirana63/guide-to-complete-statistical-analysis)

[3].
[discretize](https://www.rdocumentation.org/packages/arules/versions/1.6-4/topics/discretize)

[4].[Classification and Regression Trees (CART) in
R](https://rpubs.com/camguild/803096)

[5]. [Cluster Analysis in
R](https://rpubs.com/odenipinedo/cluster-analysis-in-R)

[6].[guide to complete statistical
analysis](https://www.kaggle.com/code/shivanirana63/guide-to-complete-statistical-analysis)
